...
const App = () => {
  ...
  let responseFilterHistorySaveTimeout: NodeJS.Timeout | null = null;
  const wrapperRef = useRef<WrapperClass | null>(null);

  async function handleSetResponseFilter(requestId: string, responseFilter: string) {
    await updateRequestMetaByParentId(requestId, {
      responseFilter,
    });

    if (responseFilterHistorySaveTimeout !== null) {
      clearTimeout(responseFilterHistorySaveTimeout);
    }

    responseFilterHistorySaveTimeout = setTimeout(async () => {
  ...
  async function handleUpdateRequestMimeType(mimeType: string | null): Promise<Request | null> {
    if (!activeRequest) {
    ...
    setTimeout(() => {
      setState(state => ({
        ...state,
        forceRefreshHeaderCounter: state.forceRefreshHeaderCounter + 1,
      }));
    }, 500);
    return newRequest;
  }
      ...
      <NunjucksEnabledProvider>
        ...
        <div className="app" key={uniquenessKey}>
          <ErrorBoundary showAlert>
            <Wrapper
              ref={wrapperRef}
              handleSetResponseFilter={handleSetResponseFilter}
